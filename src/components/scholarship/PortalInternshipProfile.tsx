...
interface Props {
  application: { user_id: string; full_name: string; email: string; telegram_username: string; twitter_handle: string };
}

export function PortalInternshipProfile({ application }: Props) {
  const { user } = useAuth();
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {profile ? "Update your internship profile details" : "Create your internship profile to appear on the market"}
          </CardDescription>
        </CardHeader>
